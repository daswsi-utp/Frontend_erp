...
const ArrivalMeanList = [
  ...
  { id: 4, name: "Base Histórica", slug: "RC" },
];
...
export const getBadgeUserStatus = (state) => {
  switch (state) {
    ...
    case "inactive":
      return (
        <Badge variant="destructive"> Inactivo, sin acceso al Sistema </Badge>
      );
    default:
      return <Badge variant="info"> {state} </Badge>;
  }
...
export const getMonthName = (date_year_month) => {
  ...
  const monthName = new Date(2021, monnthNumber - 1, 1).toLocaleString(
    "es-PE",
    { month: "long" }
  );
  return monthName;
...
export const getArrivalMeanName = (arrival_mean_id) => {
  const arrivalMean = ArrivalMeanList.find(
    (arrival_mean) => arrival_mean.id === arrival_mean_id
  );
    ...
    case "boss_approved":
      return (
        <Tooltip>
          ...
          <TooltipContent>
            <p>
              La ficha fue aprobada por el área administrativa y enviada al aula
              virtual.
            </p>
          </TooltipContent>
        </Tooltip>
